...
spiCtrl (dataSize, hasOutput) state input = (state', output)
  where
    ...
    (dataIn, finished) = case input' of
      SPI_Ctrl_in_noBusy a b -> (a, b)
      SPI_Ctrl_in_noData_noBusy a -> (0, a)
...
testObject input = message
  where
    ...
    message = dataOut#<$>(spi_Ctrl_sim clkA rstA ((SPI_Ctrl_in_noBusy<$>dataIn<*>finished)) ((7 :: Unsigned 3), True))

testInputs = [((11, low)), ((11, low)), ((11, low)), ((11, low)), 
